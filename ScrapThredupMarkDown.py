...
with open(csvfilename, 'w') as f1:
    ...
    for page in range(1, 10000000):
        ...
        if mark_down and not new_with_tags and not like_new and not gently_used and not signs_of_wear and not new_arrival:
            page_url = "https://www.thredup.com/products/women/shorts?department_tags=women&page=" + str(
                page) + "&search_tags=women-shorts&sort=Newest+First&clearance=true"
    ...
    file = open(category + str(new_with_tags) + str(like_new) + str(gently_used) + str(signs_of_wear) + str(
    mark_down) + str(new_arrival) +'_testfile.txt', 'a+')
